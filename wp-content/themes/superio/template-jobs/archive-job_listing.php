...
if ( isset( $_REQUEST['load_type'] ) && WP_Job_Board_Pro_Mixes::is_ajax_request() ) {
	if ( 'items' !== $_REQUEST['load_type'] ) {
        echo WP_Job_Board_Pro_Template_Loader::get_template_part('archive-job_listing-ajax-full', array('jobs' => $wp_query));
	} else {
		echo WP_Job_Board_Pro_Template_Loader::get_template_part('archive-job_listing-ajax-jobs', array('jobs' => $wp_query));
	}
						...
						<?php if ( is_active_sidebar( $filter_sidebar ) ): ?>
					   	...
					   	<div class="content-listing">
							<?php
								...
								echo WP_Job_Board_Pro_Template_Loader::get_template_part('loop/job/pagination', array('jobs' => $wp_query));
							?>
						</div>
					</div>
				</div><!-- .content-area -->
