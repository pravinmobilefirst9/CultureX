...
if ( !class_exists("Superio_Nav_Menu") ) {
    class Superio_Nav_Menu extends Walker_Nav_Menu {
       ...
       /**
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( $item->apus_alignment ) {
                $class_names .= ' aligned-' . $item->apus_alignment;
            }
            ...
            if ( isset($item->apus_text_label) && !empty($item->apus_text_label) ) {
                switch ( $item->apus_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'superio');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'superio');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Featured', 'superio');
                        break;
        ...
        public function generateSubMegaMenu( $item, $apus_mega_profile ) {
            if ( $apus_mega_profile ) {
                ...
                if ( $posts && isset($posts[0]) ) {
                    $post = $posts[0];
                    $content = apply_filters( 'superio_generate_post_builder', do_shortcode( $post->post_content ), $post, $post->ID);
                    $width  = $item->apus_width ? 'style="width:'.$item->apus_width.'px"':"";
                    return '<div class="dropdown-menu '.esc_attr($post->post_name).'" '.$width.'><div class="dropdown-menu-inner">'.$content.'</div></div>';
                }
            }
            return '';
        }
        ...
        public function display_icon($item) {
            if ( $item->apus_icon_image ) {
                ...
                return '<i class="'.esc_attr($item->apus_icon_font).'"></i>';
            }
        }
    }
}