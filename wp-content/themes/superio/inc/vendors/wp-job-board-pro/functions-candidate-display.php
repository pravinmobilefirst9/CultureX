...
function superio_candidate_display_featured_icon($post,$display_type = 'text') {
	...
	if ( $featured ) { ?>
        ...
        <?php }else{ ?>
            <span class="featured-text" data-toggle="tooltip" title="<?php esc_attr_e('featured', 'superio'); ?>"><?php esc_html_e('Featured', 'superio'); ?></span>
        <?php } ?>
...
function superio_candidate_display_phone($post, $echo = true) {
	$phone = WP_Job_Board_Pro_Candidate::get_display_phone( $post->ID );
	ob_start();
	if ( $phone ) { ?>
        <div class="candidate-phone">
            <?php superio_display_phone($phone, 'ti-mobile'); ?>
        </div>
    <?php }
    $output = ob_get_clean();
    if ( $echo ) {
