<?php
/**
 * Custom template tags for Superio
 *
...
if ( ! function_exists( 'superio_comment_nav' ) ) :
/**
 * Display navigation to next/previous comments when applicable.
 *
 * @since Superio 1.0
 */
function superio_comment_nav() {
	// Are there comments to navigate through?
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
	?>
	<nav class="navigation comment-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'superio' ); ?></h2>
		<div class="nav-links">
			<?php
				if ( $prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'superio' ) ) ) :
					printf( '<div class="nav-previous"><i class="fa fa-long-arrow-left" aria-hidden="true"></i> %s</div>', $prev_link );
				endif;

				if ( $next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'superio' ) ) ) :
...
if ( ! function_exists( 'superio_post_thumbnail' ) ) {
	function superio_post_thumbnail($thumbsize = '', $link = '') {
		...
		if ( is_singular('post') && is_single($post) ) {
			...
			$html .= superio_get_attachment_thumbnail($product_thumbnail_id, 'full');
			$html .= '</div>';
			...
			$html .= '<figure class="entry-thumb">';
				$html .= '<a class="post-thumbnail" href="'.esc_url($link).'" aria-hidden="true">';
						$product_thumbnail_id = get_post_thumbnail_id();
						$html .= superio_get_attachment_thumbnail($product_thumbnail_id, $thumbsize);
				$html .= '</a>';
...
if ( ! function_exists( 'superio_post_categories' ) ) {
	function superio_post_categories( $post ) {
...
if ( ! function_exists( 'superio_post_categories_first' ) ) {
	function superio_post_categories_first( $post ) {
		$cat = wp_get_post_categories( $post->ID );
		echo '<div class="list-categories">';
		foreach ( $cat as $c ) {
			$categories = get_category( $c );
...
if ( ! function_exists( 'superio_display_post_thumb' ) ) {
	function superio_display_post_thumb($thumbsize) {
        $output = superio_post_thumbnail($thumbsize);

	    return $output;
	}
}

function superio_get_attachment_thumbnail($attachment_id, $size = 'thumbnail', $icon = false, $attr = '', $wrapper = true) {
	$html = '';
	...
	if ( $image ) {
		...
		if ( superio_get_config('image_lazy_loading') ) {
			$src_layzy = superio_create_placeholder(array($width, $height));
			$attr['data-src'] = $src;
			$attr['src'] = $src_layzy;
