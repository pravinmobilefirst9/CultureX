...
class WP_Job_Board_Pro_User {
	...
	public static function registration_validation( $email, $password, $confirmpassword, $captcha = true, $check_term = true ) {
		global $reg_errors;
		...
		if ( 5 > strlen( $password ) ) {
	        $reg_errors->add( 'password', esc_html__( 'Password length must be greater than 5', 'wp-job-board-pro' ) );
	    }
	...
	public static function registration_save($user_id) {
        ...
        if ($user_role == 'wp_job_board_pro_employer') {
            ...
            if ( wp_job_board_pro_get_option('resumes_requires_approval', 'auto') != 'auto' && ($action == 'wp_job_board_pro_ajax_register' || $action == 'wp_job_board_pro_ajax_registernew') ) {
            ...
            update_post_meta($candidate_id, $prefix . 'display_name', $user_obj->display_name);
