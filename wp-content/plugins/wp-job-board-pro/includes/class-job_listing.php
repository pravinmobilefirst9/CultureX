...
class WP_Job_Board_Pro_Job_Listing {
	...
	public static function init() {
		...
		// restrict
		add_action( 'wp-job-board-pro-job_listing-query-args', array( __CLASS__, 'job_restrict_listing_query_args'), 100, 2 );
		add_action( 'wp-job-board-pro-job_listing-filter-query', array( __CLASS__, 'job_restrict_listing_query'), 100, 2 );
	}
	...
	public static function get_min_salary_html( $post_id = null, $html = true ) {
		...
		return apply_filters( 'wp-job-board-pro-get-min-salary-html', $price, $post_id );
	}
	...
	public static function get_max_salary_html( $post_id = null, $html = true ) {
		...
		return apply_filters( 'wp-job-board-pro-get-max-salary-html', $price, $post_id );
	}
	...
	public static function display_apply_job_btn( $post_id = null ) {
		...
		if ( empty($application_deadline_date) || strtotime($application_deadline_date) >= strtotime('now') ) {
			...
			if ( $apply_type == 'external' ) {
				...
				if ( !empty($apply_url) ) {
					?>
					<a href="<?php echo esc_url($apply_url); ?>" target="_blank" class="btn btn-apply btn-apply-job-external"><?php esc_html_e('Apply Now', 'wp-job-board-pro'); ?><i class="next flaticon-right-arrow"></i></a>
					<?php
				}
			} elseif ( $apply_type == 'with_email' ) {
				?>
				<a href="#job-apply-email-form-wrapper-<?php echo esc_attr($post_id); ?>" class="btn btn-apply btn-apply-job-email" data-job_id="<?php echo esc_attr($post_id); ?>"><?php esc_html_e('Apply Now', 'wp-job-board-pro'); ?><i class="next flaticon-right-arrow"></i></a>
				...
				if ( !is_user_logged_in() || !WP_Job_Board_Pro_User::is_candidate() ) {
					...
					if ( $apply_without_login == 'on' ) {
						?>
						<a href="#job-apply-internal-without-login-form-wrapper-<?php echo esc_attr($post_id); ?>" class="btn btn-apply btn-apply-job-internal-without-login"><?php esc_html_e('Apply Now', 'wp-job-board-pro'); ?><i class="next flaticon-right-arrow"></i></a>
						<?php
						echo WP_Job_Board_Pro_Template_Loader::get_template_part('single-job_listing/apply-internal-without-login');
					} else {
						?>
						<a href="javascript:void(0);" class="btn btn-apply btn-apply-job-internal-required"><?php esc_html_e('Apply Now', 'wp-job-board-pro'); ?><i class="next flaticon-right-arrow"></i></a>
						<?php
					...
					?>
					<a href="<?php echo trim($url); ?>" class="btn btn-apply <?php echo esc_attr($class); ?>" data-job_id="<?php echo esc_attr($post_id); ?>"><?php echo trim($text); ?></a>
					<?php
					if ( !$check_applied ) {
