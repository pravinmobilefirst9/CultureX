...
class WP_Job_Board_Pro_Ziprecruiter_Jobs_Hooks {

    private static $key = 'wp_job_board_pro_ziprecruiter_import';
    ...
    public static function init() {
        ...
        add_action( 'wp_ajax_wp_job_board_pro_ajax_ziprecruiter_job_import', array( __CLASS__, 'process_import_jobs' ) );
    ...
    public static function admin_menu() {
        if ( wp_job_board_pro_get_option('ziprecruiter_job_import_enable') || (isset($_POST['ziprecruiter_job_import_enable']) && $_POST['ziprecruiter_job_import_enable'] == 'on') ) {
            add_submenu_page( 'edit.php?post_type=job_listing', esc_html__('Import Ziprecruiter Jobs', 'wp-job-board-pro'), esc_html__('Import Ziprecruiter Jobs', 'wp-job-board-pro'), 'manage_options', 'import-ziprecruiter-jobs', array( __CLASS__, 'jobs_settings' ) );
        }
    }
    ...
    public static function jobs_settings() {
        ...
        <div class="wrap wp_job_board_pro_settings_page cmb2_options_page">
            <h2><?php esc_html_e('Import Ziprecruiter Jobs', 'wp-job-board-pro'); ?></h2>
    ...
    public static function import_ziprecruiter_fields() {
        $fields = array(
            'id'         => 'options_page',
            'wp_job_board_pro_title' => __( 'Ziprecruiter Jobs Import', 'wp-job-board-pro' ),
            'show_on'    => array( 'key' => 'options-page', 'value' => array( self::$key, ), ),
            'fields'     => apply_filters( 'wp_job_board_pro_ziprecruiter_job_import_fields', array(
                    ...
                    array(
                        'name'    => __( 'Per page jobs', 'wp-job-board-pro' ),
                        'desc'    => __( 'Enter per page jobs.', 'wp-job-board-pro' ),
                        'id'      => 'ziprecruiter_job_import_per_page',
                        'type'    => 'text',
                        'default' => '10',
                    ),
                    array(
                        'name'    => __( 'Radius', 'wp-job-board-pro' ),
                        'desc'    => __( 'Enter radius for search.', 'wp-job-board-pro' ),
    ...
    public static function job_meta_fields( $metaboxes ) {
        if ( wp_job_board_pro_get_option('ziprecruiter_job_import_enable') ) {
            ...
            $metaboxes[ $prefix . 'ziprecruiter_job_fields' ] = array(
                ...
                'title'                     => __( 'Ziprecruiter Job Fields', 'wp-job-board-pro' ),
                ...
                'fields'                    => array(
                    array(
                        ...
                        'id'                => WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'ziprecruiter_detail_url',
                        'type'              => 'text',
                    ),
                    array(
                        'name'              => __( 'Company Name', 'wp-job-board-pro' ),
                        'id'                => WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'ziprecruiter_company_name',
                        'type'              => 'text',
                    ),
                ),
            );
    ...
    public static function process_import_jobs() {
        ...
        $ziprecruiter_jobs = WP_Job_Board_Pro_Ziprecruiter_API::get_jobs($api_args);
        ...
        if (isset($ziprecruiter_jobs['error']) && $ziprecruiter_jobs['error'] != '') {
            ...
            foreach ($ziprecruiter_jobs as $ziprecruiter_job) {
                ...
                $existing_id = WP_Job_Board_Pro_Mixes::get_post_id_by_meta_value( WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX.'ziprecruiter_detail_url', $job_url);
                ...
                if ( $post_author ) {
                    ...
                    $user_id = WP_Job_Board_Pro_User::generate_user_by_post_name($ziprecruiter_job['company']);
                    if ( $user_id ) {
                ...
                if ( $expired_on > 0 ) {
                    ...
                    update_post_meta($post_id, WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'expiry_date', strtotime($expired_date), true);
                }
                ...
                if (!empty($ziprecruiter_job['location'])) {
                    ...
                    add_post_meta($post_id, WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'address', $ziprecruiter_job['location'], true);
                } else {
                ...
                if (!empty($ziprecruiter_job['latitude'])) {
                    add_post_meta($post_id, WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'map_location_latitude', esc_attr($ziprecruiter_job['latitude']), true);
                    $location_addrs['latitude'] = $ziprecruiter_job['latitude'];
                }

                // Insert job longitude meta key
                if (!empty($ziprecruiter_job['longitude'])) {
                    add_post_meta($post_id, WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'map_location_longitude', esc_attr($ziprecruiter_job['longitude']), true);
                ...
                add_post_meta($post_id, WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'job_referral', 'ziprecruiter', true);

                // Insert job detail url meta key
                if ( $job_url ) {
                    add_post_meta($post_id, WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'ziprecruiter_detail_url', $job_url, true);
                ...
                if ( !empty($ziprecruiter_job['company']) ) {
                    add_post_meta($post_id, WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'ziprecruiter_company_name', $ziprecruiter_job['company'], true);
                }
            ...
            $json = array(
                'status' => false,
                'msg' => sprintf(__('%s ziprecruiter jobs are imported successfully.', 'wp-job-board-pro'), count($ziprecruiter_jobs))
            );
            echo json_encode($json);
            die();
        }
    ...
    public static function get_company_name($ouput, $post) {
        ...
        $ziprecruiter_company_name = get_post_meta($post->ID, WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'ziprecruiter_company_name', true);
    ...
    public static function get_company_name_html($ouput, $post) {
        ...
        $ziprecruiter_company_name = get_post_meta($post->ID, WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'ziprecruiter_company_name', true);
        if ($job_referral == 'ziprecruiter' && $ziprecruiter_company_name != '') {
            $ouput = sprintf(wp_kses(__('<span class="employer text-theme">%s</span>', 'wp-job-board-pro'), array( 'span' => array('class' => array()) ) ), $ziprecruiter_company_name );
        }
        return $ouput;
    }
    ...
    public static function view_more_btn($post) {
        $ouput = '';
        $job_referral = get_post_meta($post->ID, WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'job_referral', true);
        $ziprecruiter_detail_url = get_post_meta($post->ID, WP_JOB_BOARD_PRO_JOB_LISTING_PREFIX . 'ziprecruiter_detail_url', true);
        if ($job_referral == 'ziprecruiter' && $ziprecruiter_detail_url != '') {
            $ouput = '<div class="view-more-link"><a class="btn btn-theme" href="' . $ziprecruiter_detail_url . '">' . esc_html__('view more', 'wp-job-board-pro') . '</a></div>';
        }
        return $ouput;
    }
}

add_filter( 'cmb2_get_metabox_form_format', 'wp_job_board_pro_ziprecruiter_modify_cmb2_form_output', 10, 3 );

function wp_job_board_pro_ziprecruiter_modify_cmb2_form_output( $form_format, $object_id, $cmb ) {
    if ( 'wp_job_board_pro_ziprecruiter_import' == $object_id && 'options_page' == $cmb->cmb_id ) {

        return '<form class="cmb-form" method="post" id="%1$s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%2$s">%3$s<div class="wp_job_board_pro-submit-wrap"><input type="button" name="submit-cmb-ziprecruiter-job-import" value="' . __( 'Import Ziprecruiter Jobs', 'wp-job-board-pro' ) . '" class="button-primary"></div></form>';
    }

    return $form_format;
}

WP_Job_Board_Pro_Ziprecruiter_Jobs_Hooks::init();