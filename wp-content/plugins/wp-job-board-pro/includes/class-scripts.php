...
class WP_Job_Board_Pro_Scripts {
	...
	public static function enqueue_frontend() {
		...
		wp_register_script( 'wpjbp-select2', WP_JOB_BOARD_PRO_PLUGIN_URL . 'assets/js/select2/select2.full.min.js', array( 'jquery'  ), '4.0.5', true );
		...
		if ( wp_job_board_pro_get_option('after_login_page_id') ) {
			$after_login_page_url = get_permalink( wp_job_board_pro_get_option('after_login_page_id') );
		} else {
		...
		if ( wp_job_board_pro_get_option('after_login_page_id_candidate') ) {
			$after_login_page_candidate_url = get_permalink( wp_job_board_pro_get_option('after_login_page_id_candidate') );
		} else {
			$after_login_page_candidate_url = $after_login_page_url;
		}

		if ( wp_job_board_pro_get_option('after_register_page_id') ) {
			$after_register_page_url = get_permalink( wp_job_board_pro_get_option('after_register_page_id') );
		} else {
			$after_register_page_url = get_permalink( wp_job_board_pro_get_option('user_dashboard_page_id') );
		}
		if ( wp_job_board_pro_get_option('after_register_page_id_candidate') ) {
			$after_register_page_candidate_url = get_permalink( wp_job_board_pro_get_option('after_register_page_id_candidate') );
		} else {
			$after_register_page_candidate_url = $after_register_page_url;
		}


		$dashboard_page_url = get_permalink( wp_job_board_pro_get_option('user_dashboard_page_id') );
		
		$login_register_url = get_permalink( wp_job_board_pro_get_option('login_register_page_id') );
