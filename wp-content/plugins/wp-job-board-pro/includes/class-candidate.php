...
class WP_Job_Board_Pro_Candidate {
	...
	public static function init() {
		...
		// restrict
		add_action( 'wp-job-board-pro-candidate-query-args', array( __CLASS__, 'candidate_restrict_listing_query_args'), 100, 2 );
		add_action( 'wp-job-board-pro-candidate-filter-query', array( __CLASS__, 'candidate_restrict_listing_query'), 100, 2 );
	...
	public static function process_apply_email() {
        ...
        if ( !empty($_FILES['cv_file']) && !empty($_FILES['cv_file']['name']) ) {
            ...
            if ( $file_data && !empty($file_data->url) ) {
        		...
        		$cv_file_url = add_query_arg(array('file_id' => $attach_id), $download_url);
            }
        }
        ...
        if ( empty($cv_file_url) ) {
        	...
        	$download_url = WP_Job_Board_Pro_Ajax::get_endpoint('wp_job_board_pro_ajax_download_cv');
    		$cv_file_url = add_query_arg(array('file_id' => $apply_cv_id), $download_url);
        }
		...
		$result = WP_Job_Board_Pro_Email::wp_mail( $author_email, $email_subject, $email_content, $headers );
		if ( $result ) {
	...
	public static function insert_applicant($user_id, $job, $cv_file_id = 0) {
        ...
        if ( $applicant_id ) {
        	...
        	if ( $cv_file_id ) {
        		...
        		$download_url = WP_Job_Board_Pro_Ajax::get_endpoint('wp_job_board_pro_ajax_download_cv');
        		$cv_file_url = add_query_arg(array('file_id' => $cv_file_id), $download_url);
        	}
