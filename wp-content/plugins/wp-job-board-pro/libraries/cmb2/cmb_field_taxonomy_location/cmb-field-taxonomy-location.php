...
/**
 * Class WP_Job_Board_Pro_CMB2_Field_Taxonomy_Location
 */
class WP_Job_Board_Pro_CMB2_Field_Taxonomy_Location {
	...
	public function __construct() {
		add_filter( 'cmb2_render_wpjb_taxonomy_location', array( $this, 'render_taxonomy_location' ), 10, 5 );
		add_filter( 'cmb2_sanitize_wpjb_taxonomy_location', array( $this, 'sanitize' ), 10, 4 );
		add_filter( 'cmb2_types_esc_wpjb_taxonomy_location', array( $this, 'escaped_value' ), 10, 3 );
		add_filter( 'cmb2_repeat_table_row_types', array( $this, 'table_row_class' ), 10, 1 );

		// Ajax endpoints.
		add_action( 'wjbp_ajax_wpjb_process_change_location', array( $this, 'process_change_location' ) );

		// compatible handlers.
		add_action( 'wp_ajax_wpjb_process_change_location', array( $this, 'process_change_location' ) );
		add_action( 'wp_ajax_nopriv_wpjb_process_change_location', array( $this, 'process_change_location' ) );
	}
    ...
    public function process_change_location() {
    	check_ajax_referer( 'wpjb-ajax-nonce', 'security' );
...
$wpjb_cmb2_field_select2 = new WP_Job_Board_Pro_CMB2_Field_Taxonomy_Location();
